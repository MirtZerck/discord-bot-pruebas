    ...
    try {
      ...
      async function getCoinsDB() {
        try {
          ...
          console.error(
            "Error al obtener las monedas de la base de datos:",
            error
          );
          throw new Error("Error al obtener las monedas de la base de datos.");
        }
      }
      ...
      async function setUserCoins(id, values) {
        try {
          await db.child("xpellCoins").child(id).set(values);
        } catch (error) {
          console.error(
            "Error al establecer las monedas del usuario en la base de datos:",
            error
          );
          throw new Error(
            "Error al establecer las monedas del usuario en la base de datos."
      ...
      async function deleteUserCoins(id) {
        try {
          await db.child("xpellCoins").child(id).remove();
        } catch (error) {
          console.error(
            "Error al eliminar las monedas del usuario de la base de datos:",
            error
          );
          throw new Error(
            "Error al eliminar las monedas del usuario de la base de datos."
      ...
      if (!arg) {
        ...
        if (keys.includes(filtro)) {
          ...
          await setUserCoins(filtro, user)
            .then((res) => {
              ...
              if (monedas > 0) {
                ...
                } ha restado **${Math.abs(monedas)}** XPELL COINS a **${
                  user.nickname
                }.**`;
                message.channel.send(
              ...
              const logsChannel = client.channels.cache.get(
                recompensasChatXpellit
              );
              if (logsChannel) {
                logsChannel.send(logMessage).catch((error) => {
                  console.error("Error al enviar el mensaje de log:", error);
                });
              }
            })
            .catch((error) => {
              console.error(
                "Error al establecer las monedas del usuario:",
                error
              );
              message.reply(
                "Ocurrió un error al actualizar las monedas del usuario. Por favor, intenta nuevamente más tarde."
              );
            });
        } else {
        ...
        await setUserCoins(filtro, user)
          ...
          .catch((error) => {
            ...
            message.reply(
              "Ocurrió un error al establecer las monedas del usuario. Por favor, intenta nuevamente más tarde."
            );
          });
      } else if (arg === "-delete") {
        // Verificar si el autor del mensaje tiene el rol de Event Manager de Xpellit
        ...
        if (keys.includes(filtro)) {
          ...
          await deleteUserCoins(filtro)
            ...
            .catch((error) => {
              console.error(
                "Error al eliminar las monedas del usuario:",
                error
              );
              message.reply(
                "Ocurrió un error al eliminar las monedas del usuario. Por favor, intenta nuevamente más tarde."
              );
            });
        } else {
      ...
      console.error("Error al ejecutar el comando systemCoins:", error);
      message.reply(
