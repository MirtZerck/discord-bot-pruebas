    ...
    try {
      ...
      const displayRank = () => {
        ...
        currentRank.forEach((val, i) => {
          puestos += `**${startIndex + i + 1}.** ${val[1].nickname}: ${
            val[1].puntos
          }\n`;
        });
        ...
        const embed = new EmbedBuilder()
          ...
          .setTitle(
            `Ranking miembros más activos en Discord - Página ${currentPage}/${totalPages}`
          )
      ...
      collector.on("collect", async (componentmessage) => {
        if (componentmessage.isButton()) {
          if (componentmessage.customId === "prevPage" && currentPage > 1) {
            ...
            componentmessage.customId === "nextPage" &&
            currentPage < totalPages
          ) {
            currentPage++;
          }

          await componentmessage.update({
            embeds: [displayRank()],
            components: [buttonComponents],
          });
        }
      ...
      console.error(
        "Error al ejecutar el comando xpellitRankingServidor:",
        error
      );
      message.reply(
