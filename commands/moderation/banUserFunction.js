  ...
  async execute(message, args) {
    ...
    try {
      ...
      message.channel.send(
        `**${member.displayName}** ha sido baneado. Razón: ${reason}.`
      );
    } catch (error) {
      console.error(error);
      message.channel.send(
        "Error al intentar banear al usuario. Por favor, verifica mis permisos y la jerarquía de roles."
      );
    }
  },
};
...
export const unbanUser = {
  ...
  async execute(message, args) {
    ...
    try {
      ...
      console.error(error);
      message.channel.send(
        "Error al intentar desbanear al usuario. Asegúrate de que la ID sea correcta y de que tengo los permisos necesarios."
      );
    }
  },
};
