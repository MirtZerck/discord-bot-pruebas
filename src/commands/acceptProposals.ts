...
export const acceptProposalCommand: Command = {
    name: "prop-list",
    ...
    async execute(message: Message, args: string[]) {
        ...
        const previousButton = createButton("anterior", "Anterior", ButtonStyle.Primary, true);
        const nextButton = createButton("siguiente", "Siguiente", ButtonStyle.Success, true);
        ...
        async function renderProposals(message: Message, proposals: any[]) {
            ...
            const menuComponents = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menuOptions);
            ...
            collector.on("collect", async (componentMessage) => {
                if (componentMessage.user.id !== message.author.id) {
                    await componentMessage.reply({ content: "No puedes interactuar con esta propuesta", ephemeral: true });
                    return;
                }
            ...
            async function handleNavigation(direction: "previous" | "next") {
                ...
                const [newPropsText, newMenuOptions] = setTextAndMenuOptionsProps(proposals);
                ...
                buttonComponents.setComponents([closeButton, previousButton, nextButton]);

                await response.edit({
                    embeds: [embedProps],
                    components: [buttonComponents, new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(newMenuOptions)],
                });
            }
            ...
            async function handleProposalSelection(componentMessage: any) {
                ...
                const embedProp = new EmbedBuilder()
                    .setAuthor({ name: `Propuesta de ${member?.displayName ?? "Unknown"}`, iconURL: member?.user.displayAvatarURL({ dynamic: true } as CustomImageURLOptions) })
                    .setTitle("Propuesta")
                    .setImage(prop.image)
                    .setDescription(`**Categoría:** ${prop.category}\n**Imagen:** ${prop.image}` || "Sin descripción")
                    .setColor(propColor)
                    .setTimestamp();

                const backButton = createButton("volver", "Volver", ButtonStyle.Secondary);
                const checkButton = createButton("aceptar", "Aceptar", ButtonStyle.Success);
                const rejectButton = createButton("rechazar", "Rechazar", ButtonStyle.Danger);

                const buttonPropComponents = new ActionRowBuilder<ButtonBuilder>().addComponents(backButton, rejectButton, checkButton);
                ...
                const collector2 = response.createMessageComponentCollector({ time: 3 * 60 * 1000 });

                collector2.on("collect", async (componentMessage2) => {
                    if (componentMessage2.user.id !== message.author.id) {
                        await componentMessage2.reply({ content: "No puedes interactuar con esta propuesta", ephemeral: true });
                        return;
                    }
                    ...
                    switch (componentMessage2.customId) {
                        case "volver":
                            ...
                            await response.edit({ embeds: [embedProps], components: [buttonComponents, menuComponents], content: "Selecciona una propuesta para ver sus detalles." });
                            collector2.stop("volver");
                            break;
                        case "aceptar":
                            await handleProposalAction(componentMessage2, "aceptada", index, prop);
                            collector2.stop("aceptada");
                            break;
                        case "rechazar":
                            await handleProposalAction(componentMessage2, "rechazada", index, prop);
                ...
                collector2.on("end", async (collected, reason) => {
                    ...
                    if (reason === "aceptada" || reason === "rechazada") {
                        ...
                        const [propText, menuOptions] = setTextAndMenuOptionsProps(updatedProposals);
        ...
        function setTextAndMenuOptionsProps(proposals: any[]): [string, StringSelectMenuBuilder] {
            const menuOptions = new StringSelectMenuBuilder().setCustomId("select").setPlaceholder("Selecciona una propuesta");

            let propText = "";
            const props = proposals.slice((currentPage - 1) * propsPerPage, currentPage * propsPerPage);
            props.forEach((p, index) => {
                const prop = p[1];
                const id = p[0];
                const i = propsPerPage * (currentPage - 1) + index + 1;
                const user = getMemberByFilter(message, prop.user)?.displayName ?? "Unknown";
                ...
                const option = new StringSelectMenuOptionBuilder().setLabel(label).setValue(id).setDescription(description);
        ...
        async function handleProposalAction(componentMessage: any, action: string, index: string, prop: any) {
            await componentMessage.deferUpdate();
            ...
            await componentMessage.reply({ content: `La propuesta ha sido ${action}.`, ephemeral: true });
        }
        ...
        async function sendClosedEmbed(message: any, response: Message, color: any, description = "Menú de propuestas cerrado.") {
            const embedOver = new EmbedBuilder()
                .setAuthor({ name: response.member!.displayName, iconURL: response.author.displayAvatarURL({ dynamic: true } as CustomImageURLOptions) })
