...
export const kickUser: Command = {
    ...
    async execute(message: Message, args: string[]) {
        ...
        try {
            ...
            message.channel.send(`**${member.displayName}** ha sido expulsado. Razón: ${reason}.`);
        } catch (error) {
            console.error(error);
            message.channel.send("Error al intentar expulsar al usuario. Por favor, verifica mis permisos y la jerarquía de roles.");
        }
    },
};
